...
def main():
  ...
  dataset = Detector(
    datadir,
    '/root/workspace/train_new_4_9.npy',
    config,
    phase='train')
  ...
  def get_lr(epoch):
    if epoch > 20 and (epoch - 1) % 10 == 0:  # args.epochs * 0.5:
      args.lr = 0.1 * args.lr
    return args.lr
  ...
  def get_lr_old(epoch):
    if epoch < 20:  # args.epochs * 0.5:
      lr = args.lr
    elif epoch <= 50:
      lr = np.power(0.1, (int(epoch) - 20) / 10) * args.lr
    else:
      lr = np.power(0.1, (60 - 20) / 10 + 1) * args.lr
    return lr

  for epoch in range(start_epoch, args.epochs + 1):
    train(train_loader, net, loss, epoch, optimizer, get_lr_origin, args.save_freq, save_dir)
