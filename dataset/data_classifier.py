...
"""
Created by Wang Han on 2018/4/24 14:23.
...
from layers.cores import iou, nms
...
class DataBowl3Classifier(Dataset):
  def __init__(self, config, phase='train'):
    ...
    self.labels = np.array(pd.read_csv(config['label_path']))

    bboxpath = config['bbox_path']
    self.phase = phase
    self.candidate_box = []
    self.pbb_label = []
    ...
    self.filenames = [os.path.join(self.data_dir, '%s_clean.npy' % idx.split('-')[0]) for idx in idcs]
    if self.phase != 'test':
